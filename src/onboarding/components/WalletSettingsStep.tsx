...
import { UserData, WalletSettings, NetworkType } from '../types';
...
const WalletSettingsStep: React.FC<WalletSettingsStepProps> = ({ setUserData, onValidationChange }) => {
  ...
  const validationRules: Partial<Record<keyof WalletSettings, { regex: RegExp; errorMessage: string }>> = {
    address: { 
  ...
  const validateField = (name: keyof WalletSettings, value: string | NetworkType | null): string => {
  ...
  const handleNetworkSelect = (network: NetworkType) => {
    setWalletSettings(prev => ({ ...prev, network }));
  ...
  useEffect(() => {
    const isValid = Object.values(errors).every(error => !error) &&
                    ...
                    walletSettings.arbitrum_rpc !== '' &&
                    walletSettings.network !== null;
            ...
            <Box key={network.chainId} sx={{ textAlign: 'center', width: '45%' }}>
              <Button
                ...
                sx={{
                  width: '100%',
                  py: 2,
                  bgcolor: walletSettings.network === network.chainId 
